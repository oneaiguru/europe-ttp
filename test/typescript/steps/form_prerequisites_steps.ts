...
Then('India-specific TTC options should NOT be available', function () {
  const forms = prerequisitesContext.available_forms || [];
  // Use explicit India form IDs instead of brittle substring matching
  // (avoiding false positives like 'training', 'admin', 'beginner', 'online', etc.)
  const INDIA_FORMS = ['ttc_application_in'];
  const indiaOptions = forms.filter((f) => INDIA_FORMS.includes(f));
