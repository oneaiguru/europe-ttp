...
// Step for XSS escaping test - renders with a benign user but the escaping step
// will test all dangerous payloads through the render function directly
When('I open the TTC portal home with dangerous payloads', async function () {
  const world = getWorld(this);
  // Set up minimal context for the test
  world.homeCountryIso = 'US';
  world.homeCountryName = 'United States';
  // The actual dangerous payload testing happens in the Then step
  // which loads the fixture and tests each payload
});
...
// TASK-206: HTML escaping test using dangerous payloads
type DangerousPayload = {
  name: string;
  input: string;
  should_not_contain: string | string[];
  should_contain: string | string[];
};

type DangerousPayloadsFixture = {
  description: string;
  payloads: DangerousPayload[];
};

function loadDangerousPayloads(): DangerousPayload[] {
  const fixturesPath = path.resolve(__dirname, '../../fixtures/dangerous-payloads.json');
  const raw = fs.readFileSync(fixturesPath, 'utf-8');
  const parsed = JSON.parse(raw) as DangerousPayloadsFixture;
  return parsed.payloads;
}

Then('the HTML output should have dangerous characters escaped', async function () {
  const payloads = loadDangerousPayloads();
  const { renderPortalHome } = await import('../../../app/portal/home/render');

  // Test each dangerous payload through the render function
  for (const payload of payloads) {
    // Render with dangerous payload as user email (escaped via escapeHtml)
    const html = renderPortalHome({
      userEmail: payload.input,
      homeCountryIso: 'US',
      homeCountryName: 'United States',
    });

    // Verify dangerous content is NOT present in raw form
    const shouldNotContain = Array.isArray(payload.should_not_contain)
      ? payload.should_not_contain
      : [payload.should_not_contain];

    for (const dangerous of shouldNotContain) {
      assert.ok(
        !html.includes(dangerous),
        `Payload "${payload.name}": should not contain "${dangerous}" in rendered HTML`
      );
    }

    // Verify escaped content IS present
    const shouldContain = Array.isArray(payload.should_contain)
      ? payload.should_contain
      : [payload.should_contain];

    for (const escaped of shouldContain) {
      assert.ok(
        html.includes(escaped),
        `Payload "${payload.name}": should contain escaped "${escaped}" in rendered HTML`
      );
    }
  }
});
