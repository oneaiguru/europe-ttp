...
import { getTestContext, initTestContext, type TestContext as SharedTestContext } from '../support/test-context';
import { getDraftContext } from './draft_steps';
...
// ============================================================================
//
// The test context is initialized once at module load time.
// This happens when Cucumber loads this step file, which occurs before
// any scenarios run. The Before hook resets context properties between
// scenarios to prevent cross-scenario contamination.
//
// Other step files should import getTestContext() from the shared module
// rather than accessing globalThis directly.
//
// ============================================================================
...
// Use the shared accessor for type-safe access
const testContext: SharedTestContext = getTestContext();
...
Before(() => {
  ...
  delete testContext.postTtcSelfEvalEmails;
  delete testContext.postTtcFeedbackEmails;
  ...
  // Reset validation context properties (for validation_steps.ts)
  delete testContext.field_errors;
  delete testContext.drafts;
});
...
Given('I have completed my applicant profile', () => {
  ...
  if (!testContext.currentUser) {
    throw new Error(
      'Step "I have completed my applicant profile" requires prior authentication. ' +
      'Add a step like "Given I am authenticated as a TTC applicant" before this step.'
    );
  }
...
Given('I have uploaded my photo', () => {
  ...
  if (!testContext.currentUser) {
    throw new Error(
      'Step "I have uploaded my photo" requires prior authentication. ' +
      'Add a step like "Given I am authenticated as a TTC applicant" before this step.'
    );
  }
...
When('I submit post-TTC self-evaluation for course starting {string} with:', (startDate: string, dataTable: DataTable) => {
  ...
  if (testContext.currentEmail) {
    if (!testContext.postTtcSelfEvalEmails) {
      testContext.postTtcSelfEvalEmails = [];
    }
    if (!testContext.postTtcSelfEvalEmails.includes(testContext.currentEmail)) {
      testContext.postTtcSelfEvalEmails.push(testContext.currentEmail);
    }
  }
...
When('I submit post-TTC feedback for {string} with:', (graduateEmail: string, dataTable: DataTable) => {
  ...
  if (!testContext.postTtcFeedbackEmails) {
    testContext.postTtcFeedbackEmails = [];
  }
  if (!testContext.postTtcFeedbackEmails.includes(graduateEmail)) {
    testContext.postTtcFeedbackEmails.push(graduateEmail);
  }
...
When('I remove {string} from the whitelist via API', (email: string) => {
  const normalized = email.toLowerCase();
  testContext.whitelist = testContext.whitelist.filter((e) => e.toLowerCase() !== normalized);
  testContext.whitelistTargetEmail = normalized;
...
When('I run the integrity report', () => {
  ...
  const selfEvalEmails = testContext.postTtcSelfEvalEmails ?? [];
  const feedbackEmails = testContext.postTtcFeedbackEmails ?? [];
  ...
  if (selfEvalEmails.length && !testContext.postTtcSubmissions?.selfEval) {
    testContext.userSummary = {
      ...testContext.userSummary,
      self_evaluation_status: 'submitted',
    };
  }
  if (feedbackEmails.length && !testContext.postTtcSubmissions?.coTeacherFeedback) {
    testContext.userSummary = {
      ...testContext.userSummary,
      co_teacher_feedback_status: 'submitted',
    };
  }
  const feedbackLookup = feedbackEmails.map((email) => email.toLowerCase());
  testContext.flaggedMissingFeedback = selfEvalEmails.filter(
    (email) => !feedbackLookup.includes(email.toLowerCase())
  );
...
// Step for submitting evaluation without matching (negative test case)
When('evaluator submits evaluation with candidate email {string}', (candidateEmail: string) => {
  // This step is for negative test cases where the email should NOT match any applicant
  // The evaluation is created with 'no_match' status and flagged for manual review
  testContext.evaluations.push({
    form_type: 'ttc_evaluation',
...
Then('the user should not be in the whitelist config', () => {
  const targetEmail = testContext.whitelistTargetEmail || testContext.currentEmail;
  if (!targetEmail) {
    throw new Error('No user email available to validate whitelist');
  }
  if (testContext.whitelist.map((e) => e.toLowerCase()).includes(targetEmail.toLowerCase())) {
    throw new Error(`User ${targetEmail} unexpectedly in whitelist`);
  }
});
...
// Negative assertion steps for fuzzy matching tests
Then('the evaluation should not be matched to any applicant', () => {
  if (!testContext.evaluations || testContext.evaluations.length === 0) {
    throw new Error('At least one evaluation should exist');
  }
  const evaluation = testContext.evaluations[testContext.evaluations.length - 1];
  if (evaluation.matching_method !== 'no_match') {
    throw new Error(
      `Expected matching_method to be 'no_match', got '${evaluation.matching_method}'`
    );
  }
});

Then('the evaluation should be flagged for manual review', () => {
  if (!testContext.evaluations || testContext.evaluations.length === 0) {
    throw new Error('At least one evaluation should exist');
  }
  const evaluation = testContext.evaluations[testContext.evaluations.length - 1];
  if (!evaluation.manual_review) {
    throw new Error('Evaluation should be flagged for manual review');
  }
});
...
Then('{string} should be flagged for missing co-teacher feedback', (email: string) => {
  if (!testContext.flaggedMissingFeedback || !testContext.flaggedMissingFeedback.includes(email)) {
    throw new Error(`${email} should be flagged for missing co-teacher feedback`);
  }
});
...
Given('I navigate to the TTC application form', () => {
  ...
  // Also set for draft context compatibility using typed accessor
  const draftCtx = getDraftContext();
  draftCtx.currentForm = 'ttc_application';
});
