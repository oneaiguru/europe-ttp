...
function getTtcOption(value) {
    return loadTestTTCOptions().find((o) => o.value === value);
}
// ============================================================================
// AUTHENTICATION & USER CONTEXT STEPS
// ============================================================================
(0, cucumber_1.Given)('I am authenticated as applicant with email {string}', (email) => {
    testContext.currentUser = getUserByEmail(email);
    testContext.currentEmail = email;
    testContext.currentRole = 'applicant';
});
(0, cucumber_1.Given)('I am authenticated as evaluator with email {string}', (email) => {
    testContext.currentUser = getUserByEmail(email);
    testContext.currentEmail = email;
    testContext.currentRole = 'evaluator';
});
(0, cucumber_1.Given)('I am authenticated as admin', () => {
...
(0, cucumber_1.Given)('I am authenticated as {string} with email {string}', (role, email) => {
    testContext.currentUser = getUserByEmail(email);
    testContext.currentEmail = email;
...
// ============================================================================
(0, cucumber_1.Given)('test TTC option {string} is available', (ttcValue) => {
...
(0, cucumber_1.Given)('TTC option {string} has display_until in the past', (ttcValue) => {
    const option = getTtcOption(ttcValue);
    if (!option) {
        throw new Error(`TTC option ${ttcValue} not found in fixtures`);
    }
...
// ============================================================================
(0, cucumber_1.Given)('I have completed my applicant profile', () => {
    // Mark profile as complete
    testContext.currentUser = { ...testContext.currentUser, profile_complete: true };
});
(0, cucumber_1.Given)('I have uploaded my photo', () => {
    // Mark photo as uploaded
    testContext.currentUser = { ...testContext.currentUser, photo_uploaded: true };
});
(0, cucumber_1.When)('I submit TTC application for {string} with:', (ttcValue, dataTable) => {
    const formData = {};
...
(0, cucumber_1.When)('I submit TTC evaluation for {string} with:', (applicantEmail, dataTable) => {
    const formData = {};
    dataTable.rows().forEach((row) => {
        formData[row[0]] = row[1];
    });
...
(0, cucumber_1.When)('I attempt to submit TTC application via API for {string}', (ttcValue) => {
    const option = getTtcOption(ttcValue);
...
// ============================================================================
(0, cucumber_1.Given)('{string} has completed TTC {string}', (email, ttcValue) => {
    testContext.graduates[email] = {
        completed_ttc: ttcValue,
        status: 'graduate',
    };
});
(0, cucumber_1.When)('I submit post-TTC self-evaluation for course starting {string} with:', (startDate, dataTable) => {
    const formData = { i_course_start_date: startDate };
    dataTable.rows().forEach((row) => {
        formData[row[0]] = row[1];
    });
    testContext.lastSubmission = {
...
(0, cucumber_1.When)('I submit post-TTC feedback for {string} with:', (graduateEmail, dataTable) => {
    const formData = { i_graduate_email: graduateEmail };
    dataTable.rows().forEach((row) => {
        formData[row[0]] = row[1];
    });
    testContext.lastSubmission = {
...
// HOME COUNTRY STEPS
// ============================================================================
(0, cucumber_1.When)('I set my home country to {string} via API', (country) => {
...
// ============================================================================
(0, cucumber_1.When)('I add {string} to the whitelist via API', (email) => {
...
(0, cucumber_1.Given)('user {string} is NOT whitelisted', (email) => {
    const normalized = email.toLowerCase();
    testContext.whitelist = testContext.whitelist.filter((e) => e !== normalized);
    testContext.whitelistTargetEmail = normalized;
});
(0, cucumber_1.Given)('user {string} is whitelisted', (email) => {
    const normalized = email.toLowerCase();
...
// ============================================================================
(0, cucumber_1.When)('I run the user summary job', () => {
    // Mock job execution
    testContext.response = {
        status: '200 OK',
...
(0, cucumber_1.When)('I run the integrity report', () => {
...
(0, cucumber_1.When)('I request the combined user application report for {string}', (email) => {
...
// ============================================================================
(0, cucumber_1.Given)('applicant {string} exists with email {string}', (name, email) => {
    testContext.applicants[email.toLowerCase()] = { name, email, role: 'applicant', home_country: 'US' };
});
(0, cucumber_1.Given)('applicant has submitted TTC application for {string}', (ttcValue) => {
...
(0, cucumber_1.When)('evaluator submits evaluation with candidate email {string} for applicant {string}', (messyEmail, applicantName) => {
...
// ASSERTION STEPS
// ============================================================================
(0, cucumber_1.Then)('the TTC application should be marked as submitted', () => {
    if (!testContext.lastSubmission) {
...
(0, cucumber_1.Then)('the form should be marked as complete', () => {
    if (!testContext.lastSubmission) {
        throw new Error('No submission was made');
    }
    if (testContext.lastSubmission.status !== 'submitted') {
...
(0, cucumber_1.Then)('the evaluation should be recorded for the applicant', () => {
    if (!testContext.lastSubmission) {
        throw new Error('No submission was made');
    }
...
(0, cucumber_1.Then)('the user should be in the whitelist config', () => {
...
(0, cucumber_1.Then)('the applicant should be able to submit within grace period', () => {
...
(0, cucumber_1.Then)('the submission should be rejected with deadline error', () => {
    if (!testContext.response) {
        throw new Error('No response exists');
    }
...
(0, cucumber_1.Then)('the form should not be marked as submitted', () => {
...
(0, cucumber_1.Then)('the evaluation should be matched to the applicant', () => {
    // In a real implementation, this would check the matching logic
    // For now, we assume success
});
(0, cucumber_1.Then)('the evaluation should count toward the applicant\'s evaluation total', () => {
    testContext.evaluationsCount++;
});
(0, cucumber_1.Then)('the evaluation should be matched via name fallback', () => {
    // Assert fuzzy name matching worked
});
(0, cucumber_1.Then)('the evaluation should be matched via fuzzy email matching', () => {
    // Assert fuzzy email matching worked
});
(0, cucumber_1.Then)('the error response should indicate grace period expired', () => {
    // Assert grace period expired error
});
(0, cucumber_1.Then)('the self-evaluation should be marked as submitted', () => {
    if (!testContext.lastSubmission) {
        throw new Error('No submission was made');
    }
    if (testContext.lastSubmission.status !== 'submitted') {
...
(0, cucumber_1.Then)('notification should be sent to {string}', (email) => {
    testContext.lastNotification = { to: email, type: 'feedback_request' };
});
(0, cucumber_1.Then)('the feedback should be linked to the graduate', () => {
    if (!testContext.lastSubmission) {
        throw new Error('No submission was made');
    }
...
(0, cucumber_1.Then)('{string} should not be flagged for missing co-teacher feedback', (_email) => {
    // Assert not flagged for missing feedback
});
(0, cucumber_1.Then)('the summary should show both self-eval and co-teacher feedback', () => {
    // Assert both types present
});
(0, cucumber_1.Then)('the user summary should show:', (dataTable) => {
    const expected = {};
    dataTable.rows().forEach((row) => {
...
(0, cucumber_1.Then)('the combined report should include both evaluations', () => {
    if (!testContext.response) {
        throw new Error('No response exists');
    }
...
(0, cucumber_1.Then)('only teacher {int} email should be in the evaluator list', (n) => {
    // Assert conditional field logic
    testContext.evaluationsList = [`teacher${n}`];
});
(0, cucumber_1.Then)('teacher 1 and {int} emails should be in the evaluator list', (n) => {
    // Assert conditional field logic
...
// UI NAVIGATION STEPS
// ============================================================================
(0, cucumber_1.Given)('I navigate to the TTC application form', () => {
    testContext.currentPage = 'ttc_application';
});
(0, cucumber_1.When)('I select {string} for "How many evaluating teachers?"', (count) => {
    testContext.numEvaluators = parseInt(count, 10);
});
// ============================================================================
// TEST MODE STEPS
// ============================================================================
(0, cucumber_1.Given)('test mode is disabled', () => {
    testContext.testModeEnabled = false;
});
(0, cucumber_1.Given)('test mode is enabled', () => {
    testContext.testModeEnabled = true;
});
(0, cucumber_1.Given)('the whitelist grace period has expired', () => {
    testContext.whitelistGraceExpired = true;
});
(0, cucumber_1.Then)('the submission should be rejected', () => {
    if (!testContext.response) {
        throw new Error('No response exists');
    }
    const status = testContext.response.status;
...
(0, cucumber_1.Given)(/^test mode is disabled \(real deadline enforcement\)$/, () => {
    testContext.testModeEnabled = false;
});
