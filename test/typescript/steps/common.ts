...
import { resetDraftContext } from './draft_steps';
import {
  userFormContext,
  configContext,
  getFormDataContext,
...
import { resetAuthContext, saveEnvState, restoreEnvState } from './auth_steps';
import { resetPDFContext } from './deterministic_pdf_steps';
import { resetBoundaryWorld } from './boundary_steps';
 ...
 */
Before(function () {
  // Save environment state before any mutations
  // This preserves external test environment configuration
  saveEnvState();
  // Reset apiContext
  ...
  // Reset boundary world state
  resetBoundaryWorld();
});

/**
 * Restore environment state after each scenario.
 *
 * This ensures that process.env mutations from auth steps don't leak
 * into subsequent scenarios, preventing cross-scenario contamination.
 */
After(function () {
  restoreEnvState();
});
