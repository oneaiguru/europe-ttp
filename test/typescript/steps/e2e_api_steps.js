...
// ============================================================================
Given('I am authenticated as applicant with email {string}', (email) => {
    testContext.currentUser = getUserByEmail(email);
    testContext.currentEmail = email;
    testContext.currentRole = 'applicant';
});
Given('I am authenticated as evaluator with email {string}', (email) => {
    testContext.currentUser = getUserByEmail(email);
    testContext.currentEmail = email;
...
Given('I am authenticated as {string} with email {string}', (role, email) => {
    testContext.currentUser = getUserByEmail(email);
    testContext.currentEmail = email;
...
// ============================================================================
Given('test TTC option {string} is available', (ttcValue) => {
...
Given('TTC option {string} has display_until in the past', (ttcValue) => {
    const option = getTtcOption(ttcValue);
    if (!option) {
        throw new Error(`TTC option ${ttcValue} not found in fixtures`);
    }
...
When('I submit TTC application for {string} with:', (ttcValue, dataTable) => {
    const formData = {};
...
When('I submit TTC evaluation for {string} with:', (applicantEmail, dataTable) => {
    const formData = {};
    dataTable.rows().forEach((row) => {
        formData[row[0]] = row[1];
    });
...
When('I attempt to submit TTC application via API for {string}', (ttcValue) => {
    const option = getTtcOption(ttcValue);
...
// ============================================================================
Given('{string} has completed TTC {string}', (email, ttcValue) => {
    testContext.graduates[email] = {
        completed_ttc: ttcValue,
        status: 'graduate',
    };
});
When('I submit post-TTC self-evaluation for course starting {string} with:', (startDate, dataTable) => {
...
When('I submit post-TTC feedback for {string} with:', (graduateEmail, dataTable) => {
...
// HOME COUNTRY STEPS
// ============================================================================
When('I set my home country to {string} via API', (country) => {
...
// ============================================================================
When('I add {string} to the whitelist via API', (email) => {
...
Given('user {string} is NOT whitelisted', (email) => {
    const normalized = email.toLowerCase();
    testContext.whitelist = testContext.whitelist.filter((e) => e !== normalized);
    testContext.whitelistTargetEmail = normalized;
});
Given('user {string} is whitelisted', (email) => {
    const normalized = email.toLowerCase();
...
When('I request the combined user application report for {string}', (email) => {
...
// ============================================================================
Given('applicant {string} exists with email {string}', (name, email) => {
    testContext.applicants[email.toLowerCase()] = { name, email, role: 'applicant', home_country: 'US' };
});
Given('applicant has submitted TTC application for {string}', (ttcValue) => {
...
When('evaluator submits evaluation with candidate email {string} for applicant {string}', (messyEmail, applicantName) => {
...
Then('notification should be sent to {string}', (email) => {
...
Then('{string} should not be flagged for missing co-teacher feedback', (_email) => {
...
Then('the user summary should show:', (dataTable) => {
    const expected = {};
    dataTable.rows().forEach((row) => {
...
Then('only teacher {int} email should be in the evaluator list', (n) => {
    // Assert conditional field logic
    testContext.evaluationsList = [`teacher${n}`];
});
Then('teacher 1 and {int} emails should be in the evaluator list', (n) => {
    // Assert conditional field logic
...
When('I select {string} for "How many evaluating teachers?"', (count) => {
