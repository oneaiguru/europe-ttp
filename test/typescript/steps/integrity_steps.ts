...
// Global declaration for integrity context
declare global {
  var integrityContext: IntegrityContext;
}

/**
 * Type guard for IntegrityContext.
 * Checks if an unknown object has the expected IntegrityContext structure.
 */
function isIntegrityContext(obj: unknown): obj is IntegrityContext {
  if (typeof obj !== 'object' || obj === null) {
    return false;
  }

  const ctx = obj as Partial<IntegrityContext>;
  return typeof ctx.integrityData === 'object' && ctx.integrityData !== null;
}

/**
 * Get or initialize the integrity context from globalThis.
 * Uses a type guard instead of `as any` for type safety.
 */
export function getIntegrityContext(): IntegrityContext {
  if (typeof globalThis.integrityContext === 'undefined') {
    globalThis.integrityContext = {
      integrityData: {},
    };
  }

  if (!isIntegrityContext(globalThis.integrityContext)) {
    throw new Error(
      'globalThis.integrityContext does not match expected IntegrityContext structure. ' +
      'This may indicate a load order issue or conflicting type declaration.'
    );
  }

  return globalThis.integrityContext;
}
...
/**
 * Normalize email address for consistent key access.
 * Ensures insertion and lookup use the same key format.
 */
...
Given('applicant has NOT uploaded required photo', function () {
  ...
  if (!getIntegrityContext().integrityData[normalizedEmail]) {
    getIntegrityContext().integrityData[normalizedEmail] = {
...
Given('applicant has started TTC application but not submitted', function () {
  ...
  const normalizedEmail = normalizeEmail(email);
  getIntegrityContext().currentEmail = normalizedEmail;
...
Given('applicant exists with email {string}', function (email: string) {
  const normalizedEmail = normalizeEmail(email);
  getIntegrityContext().applicantEmail = normalizedEmail;
  ...
  if (ctx.evaluations) {
    for (const evalEntry of ctx.evaluations) {
      if (evalEntry && evalEntry.submitted_email !== normalizedEmail) {
        evalEntry.actual_applicant_email = normalizedEmail;
...
Then('the evaluation should be flagged as mismatched', function () {
  assert.ok(getIntegrityContext().integrityData, 'No integrity data available');
