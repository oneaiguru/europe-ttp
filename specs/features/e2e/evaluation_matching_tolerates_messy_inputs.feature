Feature: Evaluation Matching Tolerates Messy Inputs
  ...
  @e2e @api @fuzzy-matching
  Scenario: Completely wrong email should not match any applicant
    Given applicant "Test Applicant" exists with email "test.applicant@example.com"
    And applicant "Other Person" exists with email "other.person@example.com"
    And applicant has submitted TTC application for "test_us_future"
    When evaluator submits evaluation with candidate email "completely.wrong@otherdomain.com"
    Then the evaluation should not be matched to any applicant
    And the evaluation should be flagged for manual review
