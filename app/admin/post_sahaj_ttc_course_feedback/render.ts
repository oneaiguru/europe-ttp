...
export const POST_SAHAJ_FEEDBACK_TITLE = 'Admin: Post Sahaj TTC Report';
export const POST_SAHAJ_FEEDBACK_TABLE_ID = 'post_ttc_feedback_summary';

export type PostSahajFeedbackRenderOptions = {
  reportingKey: string;
};

export function renderPostSahajFeedback(options: PostSahajFeedbackRenderOptions): string {
...
function get_table_data() {
  ...
  for (var _ae_raw in user_data) {
    if (user_data[_ae_raw].hasOwnProperty('post_sahaj_ttc_self_evaluation_form')) {
      ...
      var _self_evaluations = user_data[_ae_raw]['post_sahaj_ttc_self_evaluation_form'];
      ...
      for (var _te in _self_evaluations['${rk}']['evaluations']) {
        for (var _fi in _self_evaluations['${rk}']['evaluations'][_te]) {
          for (var _ve in _self_evaluations['${rk}']['evaluations'][_te][_fi]) {
            var _evaluation = user_data[_te]['post_sahaj_ttc_feedback_form'][_fi][_ve];
    ...
    if (user_data[_ae_raw].hasOwnProperty('post_sahaj_ttc_feedback_form')) {
      var _evaluations = user_data[_ae_raw]['post_sahaj_ttc_feedback_form'];
...
function load_table_data() {
  ...
  if (table) {
    ...
    table = $('#${POST_SAHAJ_FEEDBACK_TABLE_ID}').DataTable({
      data: applicant_data,
      orderCellsTop: true,
      scrollX: true,
      iDisplayLength: 50,
    ...
    $('#${POST_SAHAJ_FEEDBACK_TABLE_ID} tbody').on('click', 'td.details-control', function() {
...
function format(d) {
  ...
  if (d.email === '') {
    ...
    if (d.hasOwnProperty('evaluations')) {
      var _evaluations = d.evaluations.slice();
